...
export default function SegmentBPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/clients?segment=B')
    ...
    <div className="p-8">
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-orange-400/15 text-orange-400">Segment B</span>
          </div>
          <h1 className="text-2xl font-bold text-white">Pending Documents</h1>
          <p className="text-[#8b92a5] mt-1">
            Clients who submitted KYB in 2026 but have unresolved document requests from compliance.
          </p>
        </div>
        <button
          onClick={load}
      ...
      {loading ? (
        ...
        <>
          ...
          <ClientsTable clients={clients} segment="B" onRefresh={load} />
        </>
      )}
    </div>
  )
